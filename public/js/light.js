...
(function() {
  var add_sense, add_values, client, get_control, get_message_title, get_options, get_parametered_input, get_tile, get_tile_wrapped;
  ...
  get_tile_wrapped = function(str, js) {
    return "<div class='tiles' style='display: inline;' " + js + ">    <div class='small tile'>" + str + "</div></div>";
  };

  get_tile = function(dest, key, value) {
    var js;
    js = "onclick='window._act(\"" + dest + "\", \"" + key + "\");'";
    return get_tile_wrapped("<i class='icon-" + value['icon'] + "' " + js + "></i>", js);
  };
  ...
  get_parametered_input = function(dest, key, value) {
    ...
    str += ((function() {
      ...
      for (k in _ref) {
        ...
        switch (v.type) {
          case "string":
            _results.push("<input class='one third' placeholder='" + k + "' name='" + k + "' value='" + (v["default"] != null ? v["default"] : '') + "'/>");
            break;
          case "range":
    ...
    str += "<input type='button' onclick='window._act(\"" + dest + "\",    \"" + key + " \" + window._get_values(" + window._id + "))' value='" + key + "'></div>";
    window._id++;
    return str;
  };
  ...
  get_message_title = function(msg) {
    return "<i style='position: fixed; left: 15%; transform-origin: 0% 100%; transform: rotate(-90.0deg);'>" + (msg.destinationName.split('/')[3]) + "</i>";
  };
